<template>
  <div>
    <Head>
      <Title v-if="item">{{ item.title }}</Title>
      <Title v-else>Error</Title>
    </Head>
    <div v-if="item" class="selectedItem">
      <NuxtLink class="cec-button" href="/">Back to results</NuxtLink>
      ...
      <div class="selected-item-details">
        <div class="row">
          ...
          <div class="col-lg-6 pb-2">
            ...
            <ul>
              <li v-for="tag in item.tags" v-bind:key="tag">{{ tag.name }}</li>
            </ul>
            <h3>Meeting point</h3>
    ...
    <div v-else>
      ...
      <NuxtLink class="cec-button" href="/">Events page</NuxtLink>
    </div>
  </div>
</template>
...
const { formatDate, getTime, copyItems } = entryStore;
const route = useRoute();
const item = copyItems.find((x) => x.sys.slug === route.params.event);
let description = item ? item.excerpt : 'Error';
